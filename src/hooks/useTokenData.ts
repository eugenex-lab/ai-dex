  ...
  useEffect(() => {
    ...
    fetchData().then(() => {
      ...
      const wsConnection = new WebSocket(
        `wss://stream.binance.us:9443/ws/${cleanedSymbol.toLowerCase()}@ticker`
      );
      console.log(
        "useTokenData: Setting up WebSocket for symbol:",
        cleanedSymbol
      );

      wsConnection.onopen = () => {
        console.log(
          "useTokenData: WebSocket connected for symbol:",
          cleanedSymbol
        );
      };
      ...
      wsConnection.onmessage = (event) => {
        ...
        setData((prevData) => {
          ...
          console.log(
            "useTokenData: Updating data from WebSocket:",
            updatedData
          );
          return updatedData;
        });
